...
final class PqE2eeSessionEngine implements E2eeSessionEngine {
  ...
  static const int _maxPacketBytes = 24 * 1024 * 1024;
  ...
  @override
  Future<E2eePreKeyBundle> createPreKeyBundle({required String userId, required String keyId}) async { _local = await _preKeys.generate(userId: userId, keyId: keyId); return _local!.bundle; }
  ...
  Future<(E2eeSessionState, Uint8List)> encrypt(E2eeSessionState session, Uint8List plaintext, {required Uint8List aad}) async {
    ...
    if (packet.length > _maxPacketBytes) throw StateError('E2EE packet exceeds transport limit');
  ...
  Future<(E2eeSessionState, Uint8List)> decrypt(E2eeSessionState session, Uint8List packet, {required Uint8List aad}) async {
    ...
    while (counter < parsed.counter) {
      ...
      if (skipped.length >= _maxSkipped) skipped.remove(skipped.keys.reduce((a, b) => a < b ? a : b));
  ...
  Uint8List _handshakeBytes(String sessionId, String keyId, Uint8List ciphertext, Uint8List identityKey) { final builder = BytesBuilder(); for (final field in [utf8.encode('VIBE-E2EE-HANDSHAKE-V1'), utf8.encode(sessionId), utf8.encode(keyId), ciphertext, identityKey]) { final length = ByteData(4)..setUint32(0, field.length, Endian.big); builder.add(length.buffer.asUint8List()); builder.add(field); } return builder.toBytes(); }
  Uint8List _aad(String sessionId, int counter, Uint8List aad) { final builder = BytesBuilder()..add(utf8.encode(sessionId)); final number = ByteData(8)..setUint64(0, counter, Endian.big); builder.add(number.buffer.asUint8List()); builder.add(aad); return builder.toBytes(); }
  ...
  ({String sessionId, int counter, EncryptedPayload payload}) _decodePacket(Uint8List packet) { final decoded = jsonDecode(utf8.decode(packet)); if (decoded is! Map<String, dynamic> || decoded['v'] != 1 || decoded['sid'] is! String || decoded['n'] is! int || decoded['p'] is! Map) throw StateError('Malformed E2EE packet'); final counter = decoded['n'] as int; if (counter < 0) throw StateError('Invalid E2EE message counter'); return (sessionId: decoded['sid'] as String, counter: counter, payload: EncryptedPayload.fromJson(Map<String, dynamic>.from(decoded['p'] as Map))); }
}
