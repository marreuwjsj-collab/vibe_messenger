  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return CustomScrollView(
      slivers: [
        const SliverAppBar(
          pinned: true,
          title: Text('Vibe', style: TextStyle(fontWeight: FontWeight.w800)),
        ),
        SliverToBoxAdapter(
          child: Container(
            ...
            child: const Row(
              children: [
                ...
                SizedBox(width: 12),
                Expanded(child: Text('Vibe — общение без лишнего шума.')),
              ],
            ),
          ),
        ),
        chats.when(
          ...
          error: (error, _) => SliverFillRemaining(
            child: Center(child: Text('Не удалось загрузить чаты: $error')),
          ),
...
class _AurionTab extends StatelessWidget {
  const _AurionTab();
  @override
  Widget build(BuildContext context) => const _PlaceholderTab(icon: Icons.auto_awesome, title: 'Aurion', subtitle: 'AI внутри Vibe.');
}

class _BusinessTab extends StatelessWidget {
  const _BusinessTab();
  @override
  Widget build(BuildContext context) => const _PlaceholderTab(icon: Icons.work, title: 'Бизнес-пространство', subtitle: 'Команды, marketplace и рабочие чаты.');
}

class _ProfileTab extends StatelessWidget {
  const _ProfileTab();
  @override
  Widget build(BuildContext context) => const _PlaceholderTab(icon: Icons.person, title: 'Профиль', subtitle: 'Идентичность и настройки Vibe.');
}
          ...
          SliverFillRemaining(
            child: Center(
              child: Column(
                ...
                children: [Icon(icon, size: 48), const SizedBox(height: 16), Text(title, style: Theme.of(context).textTheme.headlineSmall), const SizedBox(height: 8), Text(subtitle)],
              ),
            ),
          ),
        ],
