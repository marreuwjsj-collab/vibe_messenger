...
void main() {
  test('ML-KEM handshake establishes matching directional E2EE chains', () async {
    ...
    final aliceBundle = await alice.createPreKeyBundle(userId: 'alice', keyId: 'alice-1');
    final bobBundle = await bob.createPreKeyBundle(userId: 'bob', keyId: 'bob-1');

    expect(aliceBundle.userId, 'alice');
    expect(bobBundle.userId, 'bob');
  ...
  test('tampered packet is rejected by AEAD authentication', () async {
    final alice = PqE2eeSessionEngine();
    final bob = PqE2eeSessionEngine();
    ...
    expect(() => bob.decrypt(bobSession, tampered, aad: aad), throwsA(isA<FormatException>()));
  });
}
